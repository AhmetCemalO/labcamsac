...
class AVTCam(GenericCam):
    ...
    def apply_settings(self):
        ...
        self.cam_handle.ExposureTimeAbs =  self.params['exposure']
        self.cam_handle.SyncOutSelector = 'SyncOut1'
        self.cam_handle.SyncOutSource = 'FrameReadout'#'Exposing'
        
        self.cam_handle.TriggerSource = self.params['triggerSource'] if self.triggered.is_set() else 'FixedRate'
        self.cam_handle.TriggerMode = 'On' if self.triggered.is_set() else 'Off'
        self.cam_handle.AcquisitionMode = self.params['acquisitionMode'] if self.triggered.is_set() else 'Continuous'
        self.cam_handle.TriggerSelector = self.params['triggerSelector'] if self.triggered.is_set() else 'FrameStart'
    ...
    # def start_streaming(self, handler):
        # """Possible to asynchronously grab images"""
        # self.cam_handle.start_streaming(handler)

    # def stop_streaming(self):
        # self.cam_handle.stop_streaming()
    
    # def frame_handler(self, cam: Camera, frame: Frame):
        #"""this would need to be implemented in camera handler
        # and passed to camera"""
        # self.frame = frame.get_buffer()
